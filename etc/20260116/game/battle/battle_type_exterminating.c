...
static void *_exterminating_init(
  ...
  if (playerc==2) CTX->playerv[1].human=2;
  else CTX->playerv[1].human=0;
  CTX->playerv[0].who=0;
  CTX->playerv[1].who=1;
  CTX->playerv[0].lastx=CTX->playerv[1].lastx=CTX->playerv[0].lasty=CTX->playerv[1].lasty=-2;
  
  // In one-player mode, the CPU player's cycle interval is based on handicap.
  // (with two humans, handicap is not used).
...
static void player_swap(void *ctx,struct player *player) {
  bm_sound(RID_sound_uiactivate,player->who?0.250:-0.250);
...
static void player_move(void *ctx,struct player *player,int dx,int dy) {
  ...
  bm_sound(RID_sound_uimotion,player->who?0.250:-0.250);
}
...
const struct battle_type battle_type_exterminating={
  .name="exterminating",
  .battletype=NS_battletype_exterminating,
  .flags=BATTLE_FLAG_ONEPLAYER|BATTLE_FLAG_TWOPLAYER,
  .foe_name_strix=56,
  .battle_name_strix=57,
