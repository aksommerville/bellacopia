...
struct modal_battle {
  struct modal hdr;
  int battleid; // NS_battle_*
  struct battle *battle;
  struct battle_args args; // What we were initialized with. (battle->args) might get adjusted by the battle.
  void (*cb)(struct modal *modal,int outcome,void *userdata);
  void (*cb_final)(struct modal *modal,int outcome,void *userdata);
  void *userdata;
  int left_name;
  int right_name;
...
static int _battle_init(struct modal *modal,const void *arg,int argc) {
  ...
  MODAL->battleid=args->battle;
  MODAL->args=args->args;
  MODAL->cb=args->cb;
  ...
  if (!(MODAL->left_name=args->left_name)) switch (MODAL->args.lface) {
    case NS_face_monster: MODAL->left_name=5; break;
    case NS_face_dot: MODAL->left_name=4; break;
    case NS_face_princess: MODAL->left_name=6; break;
  }
  if (!(MODAL->right_name=args->right_name)) switch (MODAL->args.rface) {
    case NS_face_monster: MODAL->right_name=5; break;
    case NS_face_dot: MODAL->right_name=4; break;
    case NS_face_princess: MODAL->right_name=6; break;
  }
  
  if (!(MODAL->type=battle_type_by_id(MODAL->battleid))) {
    fprintf(stderr,"%s: battle %d not found\n",__func__,MODAL->battleid);
    return -1;
  }
  if (!MODAL->type->update||!MODAL->type->render) {
    fprintf(stderr,"Battle type '%s' (%d) missing required update or render hook.\n",MODAL->type->name,MODAL->battleid);
    return -1;
  }
  if (!(MODAL->battle=battle_new(MODAL->type,&MODAL->args))) {
...
static void battle_render_play(struct modal *modal) {
  MODAL->type->render(MODAL->battle);
}
