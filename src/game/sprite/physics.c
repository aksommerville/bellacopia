...
/* Detect collisions for a sprite.
 * Caller provides the bounds to test.
 * Typically (sprite->x+sprite->hbl,etc) but can be a broader coverage area if you like.
 * Stops looking if we fill (collv).
 */
...
static int sprite_detect_collisions(
  struct coll *collv,int colla,
   ...
   */
  {
    ...
    for (;otherc-->0;otherp++) {
      ...
      double ot=other->y+other->hbt; if (ot>=b) continue;
      double ob=other->y+other->hbb; if (ob<=t) continue;
      collv[collc++]=(struct coll){ol,or,ot,ob,other};
      if (collc>=colla) return collc;
    }
  }
...
double sprite_measure_freedom(const struct sprite *sprite,double dx,double dy,struct sprite **cause) {
  ...
  struct coll collv[COLL_LIMIT];
  int collc=sprite_detect_collisions(collv,COLL_LIMIT,sprite,cover.l,cover.r,cover.t,cover.b);
  #undef COLL_LIMIT
  
  /* With the collisions gathered, identify the nearest in the direction of travel.
   */
  if (collc<1) return mag;
  struct coll *coll=collv;
  struct coll *nearest=coll;
  int i=collc;
  if (dx<0.0) {
    for (;i-->0;coll++) if (coll->r>nearest->r) nearest=coll;
    if (cause) *cause=nearest->other;
...
int sprite_move(struct sprite *sprite,double dx,double dy) {
   ...
   */
  struct sprite *other=0;
  double freedom=sprite_measure_freedom(sprite,dx,dy,&other);
  if (freedom>SMIDGEON) {
  ...
  /* Full collision.
   * If it's due to a sprite and either of them implements (collide), call those.
   */
  if (other) {
...
int sprite_test_position(const struct sprite *sprite) {
  ...
  struct coll coll;
  if (sprite_detect_collisions(
    &coll,1,sprite,
    sprite->x+sprite->hbl,sprite->x+sprite->hbr,sprite->y+sprite->hbt,sprite->y+sprite->hbb
  )>0) return 0;
  return 1;
}
