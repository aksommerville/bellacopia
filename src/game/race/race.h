...
int race_get_start_position(int *subx,int *suby,int raceid); // => mapid

/* Race accessors, valid only when a race is in progress.
 */
int race_get_lapc();
int race_get_checkpointc();
int race_get_checkpoint(double *x,double *y,int p); // Plane meters. Race begins at (p==0). Returns plane id.
int race_get_trackc();
