...
static int map_decode(struct map *dst,const void *src,int srcc) {
  ...
  while (cmdlist_reader_next(&cmd,&reader)>0) {
    switch (cmd.opcode) {
      ...
      case CMD_map_cameralock: if (!(dst->cameralock=(cmd.arg[0]<<8)|cmd.arg[1])) dst->cameralock=-1; break;
      ...
      case CMD_map_hints_override: dst->hints_override=(cmd.arg[0]<<8)|cmd.arg[1]; break;
